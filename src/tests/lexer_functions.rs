...
fn test_const_function() {
    ...
    assert_eq!(tokens.len(), 16);
    assert!(tokens.get(0).unwrap().eq(&Tok::Const));
    assert!(tokens.get(1).unwrap().eq(&Tok::Name { name: String::from("f") }));
    assert!(tokens.get(2).unwrap().eq(&Tok::Equal));
    assert!(tokens.get(3).unwrap().eq(&Tok::Function));
    assert!(tokens.get(4).unwrap().eq(&Tok::Lpar));
...
fn test_function() {
    ...
    assert!(tokens.get(0).unwrap().eq(&Tok::Function));
    assert!(tokens.get(1).unwrap().eq(&Tok::Name { name: String::from("f") }));
    assert!(tokens.get(2).unwrap().eq(&Tok::Lpar));
    assert!(tokens.get(3).unwrap().eq(&Tok::Rpar));
    assert!(tokens.get(4).unwrap().eq(&Tok::Lbrace));
    assert!(tokens.get(5).unwrap().eq(&Tok::Name { name: String::from("console") }));
    assert!(tokens.get(6).unwrap().eq(&Tok::Dot));
    assert!(tokens.get(7).unwrap().eq(&Tok::Name { name: String::from("log") }));
    assert!(tokens.get(8).unwrap().eq(&Tok::Lpar));
    assert!(tokens.get(9).unwrap().eq(&Tok::String { value: String::from("hi") }));
    assert!(tokens.get(10).unwrap().eq(&Tok::Rpar));
    assert!(tokens.get(11).unwrap().eq(&Tok::Semi));
